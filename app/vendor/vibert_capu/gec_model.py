...
class GecBERTModel(torch.nn.Module):
    def __init__(
        ...
        self.punc_str = '[' + ''.join(re.escape(x) for x in punc_dict) + ']'
        self.noop_index = self.vocab.get_token_index("$KEEP", "labels")
        self.case_indices = [
            index
            for index in range(self.vocab.get_vocab_size("labels"))
            if self.vocab.get_token_from_index(index, "labels").startswith("$TRANSFORM_CASE_")
        ]
    ...
    def get_token_action(self, token, index, prob, sugg_token):
        ...
        if prob < self.min_error_probability or sugg_token in [UNK, PAD, '$KEEP']:
            return None
        # This deployment is punctuation/capitalization-only. The upstream GEC
        # vocabulary also contains MERGE_SPACE and verb transformations; letting
        # those through turns ASR text such as "alo 1234" into "Alo1234".
        if not (
            sugg_token.startswith('$TRANSFORM_CASE_')
            or sugg_token in {'$APPEND_,', '$APPEND_.', '$APPEND_:', '$APPEND_?'}
        ):
            return None
    ...
    def _convert(self, data):
        ...
        # Separate controls: positive KEEP bias makes punctuation conservative,
        # while case bias can preserve capitalization sensitivity independently.
        if self.confidence:
            all_class_probs[:, :, self.noop_index] += self.confidence
        if self.case_confidence:
            for index in self.case_indices:
                all_class_probs[:, :, index] += self.case_confidence
